...
#define HL_HIGHLIGHT_STRINGS (1<<1)  //this is a bit mask. it is used to set the highlighting for strings
  ...
  while (i < row->rsize) {
    ...
    if (E.syntax->flags & HL_HIGHLIGHT_NUMBERS) {
      if ((isdigit(c) && (prev_sep || prev_hl == HL_NUMBER)) ||
          (c == '.' && prev_hl == HL_NUMBER)) {
        row->hl[i] = HL_NUMBER; //set hl to number
        i++;
        prev_sep = 0;
        continue;
      }
...
void editorInsertNewline() {
  if (E.cx == 0) {
    ...
    erow *row = &E.row[E.cy];
    editorInsertRow(E.cy + 1, &row->chars[E.cx], row->size - E.cx); //chars on the right of the cursor are passed to the new row
    row = &E.row[E.cy];
    row->size = E.cx;
    row->chars[row->size] = '\0';
    editorUpdateRow(row);
...
void editorOpen(char *filename) {
  ...
  editorSelectSyntaxHighlight(); //select syntax highlighting based on the file type
...
void editorFindCallback(char *query, int key) {
  ...
  //save original state of searched text color
  ...
  for (i = 0; i < E.numrows; i++) {
    current += direction;
    if (current == -1) current = E.numrows - 1;  //this ensures that we loop back to the last row if we reach the first row
    else if (current == E.numrows) current = 0;  //this ensures that we loop back to the first row if we reach the last row
...
void editorFind() {
  ...
  if (query) {
    free(query);
  } else {  //NULL query means user pressed ESC
    E.cx = saved_cx;
    E.cy = saved_cy;
...
void editorProcessKeypress() {
  ...
  switch (c) {
    ...
    case CTRL_KEY('h'):
    case DEL_KEY:
    if (c == DEL_KEY) editorMoveCursor(ARROW_RIGHT);  //tricks by moving the cursor to the right and then deleting the character
      editorDelChar();
      break;
